...
bool rbs_check_precedence_constraints(int task_id, int sequence_id, int node_id)
{
    ...
    {
		//printk("CURRENT EXECUTION STATE IS %lld and PRE is %lld, decision is true\n", current_sequence_job_ptr->nodes_finished, *ptr);
        return true;
    }
    else
    {
		//printk("CURRENT EXECUTION STATE IS %lld and PRE is %lld, decision is false\n", current_sequence_job_ptr->nodes_finished, *ptr);
        return false;
    } 
...
bool rbs_check_if_node_in_execution(int task_id, int sequence_id, int node_id)
{
    ...
    uint64_t local_execution_state = current_sequence_job_ptr->nodes_in_execution;
...
void rbs_signal_sequence(int task_id, int sequence_id, int node_id)
{
    ...
    uint64_t pre_cons = *(tasks[task_id]->kernel_pre_v + node_id - 1);
...
SYSCALL_DEFINE3(RBSinitialize_task, int,  task_id, int, number_of_nodes, int, number_of_sequences)
{
	...
	tasks[task_id]->kernel_pre_v = kcalloc(number_of_nodes, sizeof(uint64_t), GFP_KERNEL);
	tasks[task_id]->kernel_pre_h = kcalloc(number_of_nodes, sizeof(uint64_t), GFP_KERNEL);
	tasks[task_id]->kernel_sequence_heads = kcalloc(number_of_nodes, sizeof(uint64_t), GFP_KERNEL);
...
SYSCALL_DEFINE4(RBSdata_transfer, int,  task_id, uint64_t __user *, pre_v, uint64_t __user *, pre_h, uint64_t __user *, sequence_heads)
{
	int ret = 0;
	int nr_bytes = tasks[task_id]->number_of_nodes * 8;
...
{
	struct job *job_ptr = tasks[task_id]->last_job;
	
	if(job_ptr != NULL)
	{
		kfree(job_ptr->sec_guards);
		kfree(job_ptr->job_lock);
		kfree(job_ptr);
	}
	
	printk("LAST RELEASD JOB DELETED\n");
		
	/*
	while(1)
	{
		if(job_ptr->previous_job == NULL)
		{
			break;
		}
		else
		{
			job_ptr = job_ptr->previous_job;
		}
	}
	
	while(1)
	{
		if(job_ptr->sec_guards != NULL)
		{
			kfree(job_ptr->sec_guards);
		}
		if(job_ptr->job_lock != NULL)
		{
			kfree(job_ptr->job_lock);
		}
		
		if(job_ptr->next_job != NULL)
		{
			job_ptr = job_ptr->next_job;
			kfree(job_ptr->previous_job);
		}
		else
		{
			kfree(job_ptr);
	...
	*/
	...
	kfree(tasks[task_id]->kernel_pre_v);
	kfree(tasks[task_id]->kernel_pre_h);
	kfree(tasks[task_id]->kernel_sequence_heads);
	kfree(tasks[task_id]->sequences_data);
	
	printk("TASK VARIABLES DELETED\n");
...
SYSCALL_DEFINE2(RBSspeed_test, int,  num1, int, num2)
{
	return ((num1 * num2) + 10);
}
